...
class intHasher {
	...
	private $_encoding_single_big_int;
	private $_lower_max_int_value;
	private $_salt;
	private $_guards;
	
	public function __construct($salt = '', $min_hash_length = 0, $alphabet = '', $encoding_single_big_int=false) {
		...
		$this->_encoding_single_big_int = $encoding_single_big_int;
	...
	public function encode() {
		...
		foreach ($numbers as $number) {
			...
			if($this->_encoding_single_big_int && $number > $this->_max_int_value) {
				
				$tmpNumArr = str_split($number, 8);
				$numbers[$i] = (int)array_shift($tmpNumArr);
				
				foreach($tmpNumArr as $nv){
					
					if(substr($nv, 0, 1) == 0){
						
						$nv_noLeadZero = ltrim($nv, 0);
						
						if($zeroPadLen = (strlen($nv) - strlen($nv_noLeadZero))){
							
							for($pi = 0; $pi < $zeroPadLen; $pi++)
								$numbers[] = 0;
							
						}
						
						$numbers[] = (int)$nv_noLeadZero;
			...
			}elseif ((!$this->_encoding_single_big_int && $number > $this->_max_int_value) || !$is_number || $number < 0) {
	...
	public function decode($hash) {
		...
		$ret = $this->_decode(trim($hash), $this->_alphabet);
		return ($this->_encoding_single_big_int? implode('', $ret) : $ret);
	}
